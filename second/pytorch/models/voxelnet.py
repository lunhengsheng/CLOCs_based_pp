...
class VoxelNet(nn.Module):
    ...
    def forward(self, example):     #注意，这里的example是重新组织后的input
        ...
        #          12: Trv2c, 13: P2, 14: image_idx， 15： batch_image_shape]
        ...
        preds_dict[2].shape: [1, 248, 216, 4]  #preds_dict['dir_cls_preds']?
        box_preds[0][0][:2]: 
        tensor([[-0.0949, -0.0020, -0.0474,  0.0485,  0.1107,  0.1572, -0.0454,
                 -0.0430, -0.0266, -0.0420,  0.0261,  0.0649,  0.0543, -0.0712],
                [-0.4408, -0.1163, -0.0776, -0.1264,  0.0780, -0.0815, -0.1543,
                  0.1897, -0.0336, -0.0308, -0.2034, -0.0455, -0.0504,  0.0914]],
        ...
        #     exit()
        # else:
        #     # return self.predict(example, preds_dict)   #original
    ...
    def train_stage_2(self, example, preds_dict, top_predictions):
        ...
        t1 = time.time()
        t = time.time()
        batch_size = example[9].shape[0]
        batch_anchors = example[9].view(batch_size, -1, 7)

        batch_anchors_reshape = batch_anchors.reshape(1, 248, 216, 14)
        ...
        torch.cuda.synchronize()
        ttt = time.time()
        #这里在部署的时候也可以只decode anchor_mask为True的box
        ...
        t4 = time.time()
        # print("train_stage_2 t4 -t3 time: ", (t4-t3)*1000)

        predictions_dicts = []
                                                                                            #for循环之前为0.7ms左右，偶尔跳变到2ms
        for box_preds, cls_preds, dir_preds, rect, Trv2c, P2, img_idx, a_mask in zip(       #有几个batch size就会循环几次
            ...
            if self._encode_background_as_zeros:                    #True
            ...
            t5_1 = time.time()
            #print("t51 - t5 time: ", (t5_1 - t5)*1000)
            ...
            final_box_preds_camera = box_torch_ops.box_lidar_to_camera(
                final_box_preds, rect, Trv2c)                                   #0.3ms

            t_temp = time.time()
            # print("box lidar to camera time: ", (t_temp - t5_1)*1000)
            ...
            box_corners = box_torch_ops.center_to_corner_box3d(
                locs, dims, angles, camera_box_origin, axis=1)                     #0.8ms

            #print("center_to_corner_box3d time: ", (time.time() - t_temp)*1000)
            t_temp = time.time()

            box_corners_in_image = box_torch_ops.project_to_image(
                box_corners, P2)                                                    #0.25ms

            #print("project_to_image time: ", (time.time()-t_temp)*1000)

            t5_1_1 = time.time()
            #print("t5_1_1 - t5_1 time: ", (t5_1_1-t5_1)*1000)
            ...
            t5_2 = time.time()
            #print("t52 - t5_1_1 time: ", (t5_2-t5_1_1)*1000)
            t_np_1 = time.time()
            ...
            # box_2d_detector = box_2d_detection[:,:4]              #这是作者原本的意思
            ...
            tensor_index1 = np.zeros((10000000, 2), dtype=np.int64)  # 900000       #这两行0.05ms
            ...
            dis_to_lidar = dis_to_lidar.detach().cpu().numpy()          #这3行0.3ms
            ...
            iou_test,tensor_index, max_num = se.build_stage2_training_mx_v2(box_2d_preds,
                                                box_2d_detector,
                                                -1,
                                                final_scores,
                                                box_2d_scores,
            ...
            t5_4 = time.time()
            #print("t54 - t53 time: ", (t5_4-t5_3)*1000)
            ...
            t6 = time.time()
            #print("t6 - t54 time: ", (t6-t5_4)*1000)

...
from second.CLOCs_fusion.preprocess_for_fusion import preprocess_for_fusion
from second.CLOCs_fusion.preprocess_for_fusion import preprocess_for_fusion_lhs
...
def build_inference_net(config_path,
    ...
    if ckpt_path is None:
        print("build net, ------ load existing model")
...
def train(config_path,
    ...
    model_cfg = config.model.second     #voxelnet
    ...
    try:
        for _ in range(total_loop):
            ...
            for step in range(steps):
                lr_scheduler.step(net.get_global_step())   # net.get_global_step() = 0
                try:
                ...
                # assuming xyres_16.proto       # lhs 此部分待修改
                ...
                if 'anchors_mask' in example_torch:
                    ...
                    # anchors_mask = anchors_mask.bool()
                ...
                #                                                                                            detection_2d_path)
                # 3D 检测结果
                all_3d_output = net(input)

                all_3d_output_camera_dict, top_predictions, fusion_input, tensor_index = preprocess_for_fusion_lhs(net,
                                                                                                              example_torch,
                                                                                                              all_3d_output,
                                                                                                              train_flag=True)
...
def predict_kitti_to_anno(net,
    ...
    all_3d_output_camera_dict, top_predictions, fusion_input, torch_index = preprocess_for_fusion_lhs(net, example, all_3d_output, train_flag=False)

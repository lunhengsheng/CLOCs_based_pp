...
def preprocess_for_fusion(net, example, preds_dict, train_flag=True):
    ...
    batch_box_preds = preds_dict[0].view(batch_size, -1, net._box_coder.code_size)  # [bs=1, 70400/107136, 7]       # 3D net推理出来的结果；
    ...
    for box_preds, cls_preds, rect, Trv2c, P2, img_idx, img_shape, anchors, a_mask in zip(
        ...
        box_corners = box_torch_ops.center_to_corner_box3d(locs, dims, angles, camera_box_origin, axis=1)   # 八个角点
...
def preprocess_for_fusion_lhs(net, example, preds_dict, train_flag=True):
    #注意：实际batch_size应该保持为1
    ...
    for box_preds, cls_preds, rect, Trv2c, P2, img_idx, img_shape, anchors, a_mask in zip(
        ...
        num_combination = min(10000000, len(top_2d_predictions)*final_box_preds.shape[0])
        #num_combination = 10000000
        overlaps1 = np.zeros((num_combination, 3), dtype=np.float32)
        tensor_index1 = np.zeros((num_combination, 2), dtype=np.int64)
        # overlaps1[:, :] = -1
        # tensor_index1[:, :] = -1

        iou_test, tensor_index, max_num = box_np_ops.build_stage2_training_lhs(box_2d_preds.detach().cpu().numpy(),
                                                                   box_2d_detector,
                                                                   -1,
                                                                   final_scores.detach().cpu().numpy(),
                                                                   box_2d_scores,
        ...
        tensor_index_tensor = torch.LongTensor(tensor_index)
        iou_test_tensor = iou_test_tensor.permute(1, 0)
        iou_test_tensor = iou_test_tensor.reshape(1, 3, 1, num_combination)     #torch.Size([1, 4, 1, 10000000])
        tensor_index_tensor = tensor_index_tensor.reshape(-1, 2)                #torch.Size([10000000, 2])

        if max_num == 0:
            non_empty_iou_test_tensor = torch.zeros(1, 3, 1, 2)
            non_empty_iou_test_tensor[:, :, :, :] = -1
            non_empty_tensor_index_tensor = torch.zeros(2, 2)
            non_empty_tensor_index_tensor[:, :] = -1
        else:
